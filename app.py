...
def main():
    ...
    if st.button("Generate Answers"):
        ...
        if uploaded_file:
            questions.extend(uploaded_file.read().decode("utf-8").splitlines())
        
        if user_question:
            questions.append(user_question.strip())
            ...
            for question in questions:
                if question.strip():  # Ensure the question is not empty
                    try:
                        ...
                        st.error(f"An error occurred while generating answer for '{question}': {e}")
